...
- `size_valid` — `size` is one of `1` / `2` / `3` / `5` (see **Task size** — `> 5` means the task should be split into sub-tasks); re-evaluate against the backfilled `size` unless the honest estimate exceeded `5`, in which case this still fails. When it fails *for that reason*, the `# promoter:` comment step 3 writes must record `scope exceeds size 5 — split into sub-tasks`, **not** the `size_valid` / `required_fields_present` failure — same precedence the Linear path applies. Otherwise the card reports "fill in a number" when the actual instruction is "split this task"
