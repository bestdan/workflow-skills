...
**Backfill missing estimates.** Before scoring, backfill two fields on every `status: new` candidate **except one held on an unresolved blocker** (those get no write at all — see step 3). Otherwise this is unconditional: backfill even a candidate that will fail some other check and land in `needs_refinement` anyway, so the human has less to redo when it gets there:
...
- `size_valid` — `size` is one of `1` / `2` / `3` / `5` (see **Task size** — `> 5` means the task should be split into sub-tasks); re-evaluate against the backfilled `size` unless the honest estimate exceeded `5`, in which case this still fails. When it fails _for that reason_, the `# promoter:` comment step 3 writes must record `scope exceeds size 5 — split into sub-tasks`, **not** the `size_valid` / `required_fields_present` failure — same precedence the Linear path applies. Otherwise the card reports "fill in a number" when the actual instruction is "split this task"
