...
Shared definitions used by `linear-add.md` and `linear-list.md`. This file has no commands of its own — it only defines the config schema, the preflight pattern, and the kanban mapping table that every Linear-handled command needs.
...
Every Linear-handled command starts the same way. Failure messages are identical across `/add-todo` and `/list-todos` so the user sees the same guidance regardless of which command surfaced the error.
...
2. (Per-verb files do additional setup — e.g. `list_workflow_states` for `/list-todos`.) The shared part is just the team check.
...
- Linear's GitHub integration moves the issue to the `completed` state on PR merge via `Closes <KEY>`.

The `auto-claimed`, `blocked`, and `In Review` rows above describe states the kanban model can express but no Linear-handled command in this PR writes them — they're populated by external tools (the GitHub integration, manual edits in Linear) or by future commands.
