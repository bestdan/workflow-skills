...
linear:
  ...
  # workspace-wide cap Linear's free plan enforces. It counts EVERY non-archived issue,
  # whatever its state: Backlog, Todo, In Progress, Done and Canceled all count, and
  # ARCHIVING is the only thing that removes one. Completing or cancelling an issue does
  # not. Over the cap, Linear refuses to CREATE new issues; existing ones still update.
  # Set it to YOUR plan's cap. 250 is the free-plan figure as of 2026-09; Linear's
  # billing page is the authority for it, not this file. Must be an integer 0-250
  # (the domain's own ceiling is that same observed figure -- see the note below).
  # Default 250. Set to 0 to disable the report entirely (e.g. on a paid plan with no cap).
  # Any other out-of-range/non-numeric value disables the report for that run with a warning.
  # It GATES nothing: step 7 reports headroom and promotes either way, so there is no
  # fail-open/fail-closed axis here at all.
...
- `active_issue_quota` is optional and lives under `linear:`. It is read only by `/promote-tasks` (`linear-promote.md` step 7) as the workspace-wide cap on **non-archived issues of every state** — Backlog, Todo, In Progress, Done and Canceled all count toward it, and **archiving is the only way to remove one**. The name is historical and reads narrower than the thing it measures. **This key is where the number lives, and the repo cannot verify it.** Linear exposes no plan or quota field — checked 2026-09-16, the MCP's `get_workspace` returns only id/name/url and no documented `Organization` field carries a limit — so **`250` here is a dated observation of the free-plan cap (2026-09), not a fact this repo can re-derive.** Linear's billing page is the authority; set the key to your own plan's cap and treat every `250` elsewhere in these files as sourced from here. **Must be an integer `0`–`250`.** Default `250`; `0` disables the report. Any other value (negative, non-integer, non-numeric, or `> 250`) is invalid and disables the report for that run, with a warning, rather than doing arithmetic against an uncheckable ceiling. **It gates nothing** — step 7 reports headroom and promotes either way, because a promotion moves an issue between two states that both count and so spends no quota. The fail-open/fail-closed comparison with `orphan_claim_hours` no longer applies: this key withholds no write in any configuration. What the cap actually binds is issue **creation** (`/add-task`, `/push-plan`), which has no check of its own today.
...
| `needs_review`     | `started`         | `In Progress` (or `In Review` if the team has one) | — (the open PR is the review signal via the explicit `links` attachment from the tracker execute path; Linear's GitHub integration may also create this link where it is live, but nothing here relies on it) |
| `done`             | `completed`       | `Done`                                             | —                                                                                                                                                                                                             |
...
- The issue reaches `completed` via the reconciler verbs — `/sweep-for-complete` or `/reconcile-tasks`, both built on the `/complete-task` primitive — which verify that the issue's **own** `links`-attached PR merged before completing it. Linear's GitHub integration is not what this plugin relies on; where it is live it may also complete the issue on merge, via the `Closes <identifier>` line `linear-claim.md` mandates, and that is expected rather than a false closure (see `linear-claim.md`, "Whether Linear's integration is live"). The over-closing that motivated the reconciler verbs came from a repo-local GitHub Actions workflow that scraped `<TEAM>-NNN` ids out of PR title and body text on merge and swept every match to `Done`. `/do-tasks` adds the explicit `links` attachment to the issue on PR open (not dependent on branch-name matching); that attachment is what makes the reconciler verbs' verification possible.
...
> **Hard rule for the tracker execute path: it never moves a Linear issue to a `completed`- or `canceled`-type workflow state.** Completion is driven by the reconciler verbs (`/complete-task`, `/sweep-for-complete`, `/reconcile-tasks`), never by this path itself. A live Linear integration may additionally complete the issue on merge; that is expected, and it is not licence to set the state here. If you are about to call `save_issue` with a `completed`-type `state` from `linear-claim.md`, you have a bug — stop.
