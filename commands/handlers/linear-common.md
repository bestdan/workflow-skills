...
> **Hard rule for the tracker execute path: it never moves a Linear issue to a `completed`- or `canceled`-type workflow state.** Completion is driven by the reconciler verbs (`/complete-task`, `/sweep-for-complete`, `/reconcile-tasks`), not by Linear's GitHub integration, which is inert. If you are about to call `save_issue` with a `completed`-type `state` from `linear-claim.md`, you have a bug — stop.
