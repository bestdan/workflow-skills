...
- Per-project override keys are `wip_limit`, `max_estimate`, and `repo` (which consumers read `repo` is stated once, on the `repo` bullet under "Resolve configured projects"). `team`, `base_branch`, `default_priority`, `api_key`, `api_key_ref`, and `api_key_resolver` remain global. `api_key_ref` points at a secret (a full-account bearer token) — its canonical home is the gitignored `.task-config.local.yml`, not the shared `.task-config.yml` (see `commands/task-config.md` → "Local override"). `api_key` (a raw key) and `api_key_resolver` (which names a program) are stricter still: both are honored **only** from `.task-config.local.yml` or the environment, and are refused outright in the committed config (see "Key resolution").
