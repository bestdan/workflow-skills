...
It does git plumbing, so it uses an agent + the remote/subagent/local cascade. The `linear` and `gh-issue` tracker handlers also dispatch **remote** sessions, but only for batch `/do-tasks --all` (see `commands/do-tasks.md` §3 "Tracker-batch subroutine" and its gh-issue instantiation in §4); their single paths, and the MCP handler (`jira`), are single foreground calls.
