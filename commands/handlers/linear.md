...
3. **Query issues.** Call `mcp__claude_ai_Linear__list_issues` with:
   ...
   - `includeArchived`: `false`
   - Limit: ~100. If the response indicates more issues exist, render a `(showing first 100 of N — refine with linear.default_project)` note at the end of the summary line. Pagination/cursor handling is out of scope for v1.

   The goal is "everything still active in the team's kanban." Pull all non-archived issues in the `backlog`, `unstarted`, `started`, and recently-`completed` state types. If the MCP tool requires more specific filters (e.g. by state id or label) to avoid over-fetching, resolve the team's workflow states once via the same lookup you use in create step 4, then query each relevant state type.
...
5. **Render** as stacked vertical sections in this fixed order, omitting empty sections:
   ...
   Use the same `## <section> (N)` header, single-line bullet, `---` separator layout as the file-based path in `commands/list-todos.md` step 4. Sort within each section by Linear priority (1=urgent > 2=high > 3=medium > 4=low > 0=none), then by `updatedAt` (oldest first).
   ...
   - `blocked: <reason>` — pull the latest comment body's first line on the issue when the `blocked` label is set; truncate at 60 chars. If no comment exists, render bare `blocked`.
   - `PR #<n> open` — for `needs_review` cards, the linked PR number from Linear's GitHub integration.
