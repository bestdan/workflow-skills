...
> **Hard rule for every phase below: the tracker path never moves a Linear issue to a `completed`- or `canceled`-type workflow state.** Completion is now driven by the reconciler verbs — `/complete-task` (the primitive), `/sweep-for-complete` (the merge-verified sweep), and `/reconcile-tasks` (the bounded reconciler built on both) — never by this file's own flow, whether or not Linear's GitHub integration is live in your workspace (see "Whether Linear's integration is live" below). Where that integration **is** live it may additionally close an issue on merge, via the `Closes <identifier>` line this file mandates below; that is expected, not a bug. If you are about to call `save_issue` with a `completed`-type `state` from this file, you have a bug — stop.
