...
1. **Resolve project scopes.** Call the **"Resolve configured projects"** helper from `linear-common.md` (same as the floor) to get the list of scopes `{ id, name, wip_limit, max_estimate }`. Apply the same **specific-pin shortcut** as the floor's step 3 when `/do-tasks` was called with `--project <name|id|unassigned>`.
