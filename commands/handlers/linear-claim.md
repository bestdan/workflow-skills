...
> **API key enables the read fast-path.** Setting `linear.api_key_ref` (see `linear-common.md` "Config block") lets this section skip the MCP fan-out below and resolve candidates in one GraphQL call via `linear-ready.py`. The configured ref reaches the script exactly as `linear-common.md` → "Key resolution" describes — `opx run --env 'LINEAR_API_KEY=<ref>' -- python3 …` when `opx` is on `PATH` (an approval dialog gates the read, no `op` session for the agent's subshell to inherit), else the `LINEAR_API_KEY_REF='<ref>' python3 …` form, which needs an authorized `op` session: run `op signin` in **your own** terminal once (it stays usable from the agent's subshell, and lapses after ~30 minutes idle). Exporting `LINEAR_API_KEY=$(op read "$LINEAR_API_KEY_REF")` in the launching terminal also still works and takes precedence. A headless run can instead set `$OP_SERVICE_ACCOUNT_TOKEN` plus `$LINEAR_API_KEY_REF` and let the script resolve the key itself (`opx` cannot serve a headless run — no UI, fails closed). `claude.ai`/Claude Code cloud sandboxes never have this key set, so they stay on the MCP floor by design (see the security-boundary note below).
