...
> **API key enables the read fast-path.** Setting `linear.api_key_ref` (see `linear-common.md` "Config block") lets this section skip the MCP fan-out below and resolve candidates in one GraphQL call via `linear-ready.py`. To use it from an interactive session, `export LINEAR_API_KEY=$(op read "$LINEAR_API_KEY_REF")` into the **launching terminal** before starting Claude Code — inheriting an already-resolved env var is fine; it's invoking `op` from _inside_ the agent's tool-spawned subshell that fails under 1Password desktop-app integration (see `linear-config.md` "Archive key"). A headless run can instead set `$OP_SERVICE_ACCOUNT_TOKEN` plus `$LINEAR_API_KEY_REF` and let the script resolve the key itself. `claude.ai`/Claude Code cloud sandboxes never have this key set, so they stay on the MCP floor by design (see the security-boundary note below).

**One mechanism: try the fast path, fall back to the floor.** At the top of the **ranked** candidate search — the direct-identifier path (`/do-tasks PRE-12`) always stays on the MCP floor, see step 7 — if `Bash` is available, attempt the **GraphQL fast-path** first. On **any** non-zero exit from `linear-ready.py`, or stdout that doesn't parse as the expected JSON object, log one debug line (`Fast-path unavailable (<reason>) — falling back to MCP floor.`) and run the **MCP floor** (the shared preflight below plus steps 2–6) instead. There is no separate mechanism and no independent `[ -n "$LINEAR_API_KEY" ]` pre-check gating this — `linear-ready.py` itself exits fast and non-zero when no key is resolvable, so the fallback **is** the gate. An explicit env pre-check would misgate the headless case where `$OP_SERVICE_ACCOUNT_TOKEN` + `$LINEAR_API_KEY_REF` are set but `$LINEAR_API_KEY` itself is not — that case must still attempt the fast path. A host with no `Bash` tool falls to the floor by construction (nothing to gate). The fast-path is a pure optimization, never a hard dependency: by construction (both consumers implement `linear-common.md` → "Ready-candidate selection" identically) the two paths select the same candidates.

> **This gate is also the security boundary.** A Linear personal API key (what `linear.api_key_ref` points at) is a full-account bearer token — anyone holding it can read and write everything the key's owner can in Linear. It must **never** be injected into a `claude.ai`/Claude Code **cloud** sandbox. Cloud sessions never set `$LINEAR_API_KEY`/`$LINEAR_API_KEY_REF`, so even where a cloud host is `Bash`-capable and attempts `linear-ready.py`, the script exits non-zero before any GraphQL request (no key resolvable) and the run falls to the MCP floor (OAuth-scoped, no raw key) by design — the guarantee is that the key is never present, not that the script is never invoked. Do not "fix" this by wiring the key into cloud config.
...
1. **Resolve project scopes.** Call the **"Resolve configured projects"** helper from `linear-common.md` (same as the floor) to get the list of scopes `{ id, name, wip_limit, max_estimate }`, and pass each scope's real `id` as `--project` (omit for the whole-team `id: null` scope). The fast path covers **only** configured-project and whole-team scopes: `linear-ready.py` has no Unassigned-bucket exclusion mode (that bucket is resolved by client-side subtraction, a floor-only mechanism), so **never** serialize the `__unassigned__` sentinel as a `--project` value. If the run needs the Unassigned bucket — a `--project unassigned` pin, or the no-flag ranked path on a repo with **1+ configured `linear.projects`** (where the floor also runs its Unassigned pass) — **fall back to the MCP floor** for that search so unassigned work isn't missed. Likewise, when `/do-tasks` was invoked with a specific `--project <name|id>` pin (configured or live/unconfigured), **fall back to the MCP floor** for that search — its "Specific pin shortcut" (via `linear-common.md` "Resolve claim scope" step 1) scopes discovery to exactly the pinned project, which the fast path's full configured-scope list does not. With **0 projects configured** (the default), the single whole-team scope already spans everything, so the fast path is fully equivalent to the floor and needs no such fallback.
...
4. **Query — once per resolved scope.** For each scope from step 3, call `<linear-mcp>__list_issues` passing **every server-side filter the tool supports**, so the MCP does as much of the work as possible before anything client-side runs:
   ...
   `list_issues` has **no server-side filter for `estimate` or for label exclusion** (`auto-claimed` / `human-approval-requested` / `blocked`), so those gates — and the assignee gate — stay client-side in step 5. That gap is exactly _why_ this floor stays a fatter, multi-call fan-out where the fast path collapses to one filtered GraphQL request.
...
5. **Filter.** Apply the gates from `linear-common.md` → "Ready-candidate selection" (the `<max>` is the candidate's resolved per-project `max_estimate` from step 3, default `3`; `<linear-mcp>__get_user` with no args returns the viewer for the assignee gate) — this is where the `estimate`/label-exclusion/assignee gates step 4 couldn't push server-side actually run. (Linear's native `estimate` uses the same Fibonacci scale as our task `size` — see "Task size" in skills/task/SKILL.md — so these thresholds select tasks small enough to finish in one session.)
