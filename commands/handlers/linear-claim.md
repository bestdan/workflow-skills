...
> **Hard rule for every phase below: the tracker path never moves a Linear issue to a `completed`- or `canceled`-type workflow state.** Completion is now driven by the reconciler verbs — `/complete-task` (the primitive), `/sweep-for-complete` (the merge-verified sweep), and `/reconcile-tasks` (the bounded reconciler built on both) — not by Linear's GitHub integration, which is **inert** (see "Why the integration is inert" below). If you are about to call `save_issue` with a `completed`-type `state` from this file, you have a bug — stop.
...
### Why the integration is inert

It was never switched off as a decision. Linear's GitHub App is installed under a GitHub identity that cannot see the repositories this plugin manages, so it has never fired on them. Verified 2026-09-05: `bestdan/finplan` and `bestdan/workflow-skills` both carry **zero** webhooks, no issue carries a Linear-created linkback, and `bestdan/finplan#997` — whose head branch `dan/pre-381-add-a-favicon-to-the-marketing-site` embeds an issue id, which is the one link form that does close on merge — merged with `PRE-381` still sitting in `In Review`.

The over-closing that earlier revisions of this file attributed to Linear came from somewhere else: `bestdan/finplan` carried its own `.github/workflows/linear-close-on-merge.yml`, which ran `grep -oE '\bPRE-[0-9]+\b'` across the PR title and body on every merge to `main` and moved every match to `Done`. `bestdan/finplan#1157` deleted it.

### What Linear does when the integration is live

Per Linear's own documentation, a PR links to an issue three ways, and only two of them close it on merge:
...
5. **Gates and dropped issues.** The script already applied the gates and rank from `commands/handlers/assets/_linear_rank.py` (imported as `gate()`/`rank_key()` — see that module's header for the six-gate table) — do not re-filter. Report each `dropped` entry the same way the floor reports a gate drop: same reason strings, by construction (the script emits the identical fixed strings from that table).
...
5. **Filter and rank — one call.** Call `<linear-mcp>__get_user` **once**, cache the result as the **viewer id** for the rest of this run — "Pre-flight" step 5 and "Claim the issue" step 3 reuse this cached id (on the fast path the cache is `meta.viewer.id`, per "Fast path" step 4). Then feed step 4's tagged, unioned candidate list to `linear-rank.py`, which applies the `linear-common.md` → "Ready-candidate selection" gates and rank (see `commands/handlers/assets/_linear_rank.py` for the six-gate table) in one pass — this is where the `estimate`/label-exclusion/assignee gates step 4 couldn't push server-side actually run:

   ```bash
   echo '<step-4 tagged candidates, JSON array>' | python3 "${CLAUDE_PLUGIN_ROOT}/commands/handlers/assets/linear-rank.py" \
     --max-estimate <linear.max_estimate, default 3> --viewer-id "<cached viewer id>"
   ```

   Each candidate's own `project.max_estimate` (tagged in step 4) overrides `--max-estimate` for that one issue, so a single call correctly gates a multi-scope union where scopes have different maxes. If `$CLAUDE_PLUGIN_ROOT` is unset and the path doesn't resolve, Glob `**/handlers/assets/linear-rank.py`. Parse stdout as `{ candidates: [...], dropped: { "<id>": "<reason>", ... } }` — `candidates` is already gated **and** ranked (priority urgent→low, none last, then `updatedAt` ascending); report each `dropped` entry with its reason the same way the fast path does (same fixed strings, by construction — both paths call the same `gate()`). This is the floor's full enforcement of the assignee gate — it is **not** weaker than the fast path's `isMe`, only relative to a different viewer (see the same-user note in "Find candidates"). (Linear's native `estimate` uses the same Fibonacci scale as our task `size` — see "Task size" in skills/task/SKILL.md — so these thresholds select tasks small enough to finish in one session.)
...
7. **Return** the ranked list to the **Pre-flight** phase below. Each entry needs: `id`, `identifier`, `title`, `priority`, `estimate`, `description` (the MCP floor carries this; fast-path candidates omit it — "Judge feasibility" backfills it via `get_issue` for the one claimed candidate, same as today), `labels`, `url`, **`branchName`** (Linear's published git branch name — used verbatim when `/do-tasks` branches), **`project`** (the `{ id, name, wip_limit, max_estimate }` scope it came from — `id: null` for whole-team — so the WIP gate checks the right per-project cap), and the resolved `id`s for: team, current `state`, target `started`-type `state` (see "Claim the issue" below). The execute path takes candidates in ranked order and, for each, runs **pre-flight → claim → judge feasibility** — the feasibility read now happens _after_ the claim, while the lock is held.
   ...
   **Direct-identifier path (`/do-tasks PRE-12`) always runs on the MCP floor, never the fast path** — the fast path targets the ranked search over many candidates, while a known identifier is a single targeted read, so this path stays on `<linear-mcp>__get_issue`. If the `/do-tasks` argument was a specific identifier, run the MCP floor's **Preflight (step 1)** and **steps 2–3** — the cached state map from step 2 is still needed by "Claim the issue" and "Move to review", and step 3 resolves the configured scopes — then **skip only step 4's per-scope query and the step 5 ranked filter/rank**, and call `<linear-mcp>__get_issue` with that identifier. **Determine its `project` scope** by matching the issue's `projectId` against the resolved configured-projects list (step 3): if it matches a configured project, use that scope; **otherwise** (no project, or a project not in the list) use the **shared Unassigned bucket** exactly as defined in `linear-common.md` "The Unassigned bucket" (`{ id: "__unassigned__", name: "Unassigned", wip_limit: <linear.unassigned_wip_limit if set else top-level wip_limit>, max_estimate: <linear.max_estimate, default 3>, unassigned: true }`) — **not** a per-issue synthesized scope. All out-of-project direct picks therefore share one cap, counted by subtraction in `do-tasks.md` "Pre-claim WIP gate" (never by passing the sentinel as a `projectId`). If 0 projects are configured (no Unassigned bucket exists), the step-3 list is the single whole-team scope and the issue simply uses that. Apply the filters from step 5 to that one issue using **that scope's** `max_estimate`; if it fails any gate, return the failure reason rather than the issue — with one exception. A step-5 drop whose reason is `human-approval-requested` is the hold a human placed on the issue, and the person naming it is who the hold is for: apply the held-issue override in `commands/handlers/attendedness.md` (that file owns the prompt and the hard negatives; on decline, surface the reason and stop as before). On **Claim anyway** proceed with the issue and keep the `human-approval-requested` label in the label set "Claim the issue" step 6 writes — the override claims the issue, it does not release it to the loop. Every other gate (`auto-claimed`, `blocked`, the estimate gates, an assignee that is not the viewer) still refuses a direct identifier — `/do-tasks` surfaces the reason and stops.
