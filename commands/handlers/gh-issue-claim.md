...
**`--all` / `-n N` without `--claim-only`** is **not** driven from this file. `commands/do-tasks.md` §4 "gh-issue batch" ranks and selects the dependency-ready candidates itself, then dispatches one remote session per selected issue, each running the flow above against its one issue number **with §4's two batch substitutions** — "Find candidates" skipped, and the claim taken on `claim-lock.md`'s comment-token election rather than the ref acquire (the same two this file's opening paragraph names). Everything else runs as written. In that session every pre-claim gate is verification-only: a blocked dependency, a pre-flight trip, a feasibility reject, or a lost claim **stops and reports** rather than advancing to another candidate — advancing would put two dispatched sessions on one issue. `--all` / `-n N` with `--local` never dispatches: it caps the batch at 1 and runs the single highest-ranked issue through the default flow in the current session.
