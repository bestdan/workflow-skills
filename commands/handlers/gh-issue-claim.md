...
1. **Re-read** the chosen issue (`gh issue view <n> --json assignees,labels [--repo <repo>]`). If it now has an assignee, **another session beat you** — return `race`, fall back to the next candidate (on a direct pick: stop and report). If its rung is not `status:2_ready`, or it does not carry `auto:eligible`, a human is holding it back from automation. On the **ranked path** that is also `race`. On a **direct `/do-tasks <#n>` pick** apply the held-issue override in `commands/handlers/attendedness.md` — that file owns the prompt and the hard negatives; this step owns only the message: on decline report `#<n> is <its status: and auto: labels> — withheld from automation, no issue claimed` and stop. A session a dispatcher handed a pinned number is not a direct pick (`do-tasks.md` §4 declares it unattended). This is a cheap early-out, not the lock; note the wall-clock time of this read as `T_unclaimed` (the fallback election in `claim-lock.md` needs it). Keep the label list — step 3 needs it, and on an override it is what carries the `auto:human-review-needed` hold through.
