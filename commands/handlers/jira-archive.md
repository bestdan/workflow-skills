...
1. **Preflight.** Run the Atlassian MCP preflight from `jira.md` step 1 (resolve
   `cloudId` from the configured `site`). Read the `jira:` block for `project` and
   the optional **`archive_status`** key (the name of the terminal-category status
   to move archived issues to — see `jira-config.md` → "Archive status").
...
3. **Find candidates.** Query terminal issues older than the threshold via JQL:

   ```text
   project = "<project>" AND statusCategory = Done AND resolutiondate <= "-<N>d"
   AND status != "<archive_status>" ORDER BY resolutiondate ASC
   ```

   Call `<atlassian-mcp>__searchJiraIssuesUsingJql` with `cloudId`, that `jql`,
   `fields: ["summary","status","resolutiondate"]`, and a sensible `maxResults`.
