...
> - **Promote / claim (added after this audit was scoped):** `jira-promote.md` step 3 (`project` + `statusCategory = "To Do"` + per-key `status !=`), the WIP gate (`statusCategory = "In Progress"`), and `jira-claim.md` step 2 (`status = "<ready_status>" AND assignee IS EMPTY AND Flagged IS EMPTY AND (labels IS EMPTY OR labels NOT IN ("human-approval-requested", "blocked"))`) are all server-side. `jira-claim.md` step 3 keeps a **defensive** post-fetch backstop for the same label drop (guards against JQL index lag); `assignee IS EMPTY` already excludes claimed work, so the label clause is belt-and-suspenders for the edge case where a board leaves those labels in the ready lane. The `labels IS EMPTY` arm in the JQL clause is **required** — JQL `NOT IN` does not match issues whose field is empty, so omitting it would drop unlabeled issues (the common case in the ready lane).
