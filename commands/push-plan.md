...
1–2 write nothing new — and in particular **do not** set or overwrite the
`description` when reusing a project (a recorded `tracker_id` or
`default_project` may point at a shared/pre-existing project; the overview is
written **only on create**). When the container is reused, the overview has no
description home, which §7 reports and which keeps the epic file from being
deleted.
