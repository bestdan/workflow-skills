...
| process — batch (`/do-tasks --all`) | yes       | yes        | no     | yes      |
...
`/do-tasks` is the single execute verb across handlers: executes a single task by default, `--all` / `-n N` for batch dispatch on `repo-pr`, `linear`, and `gh-issue` (each dispatches one remote session per dependency-ready task or issue, bounded by the WIP limit). On `jira` it claims and executes one issue in the current session.
...
- **The handler's supported and unsupported verbs**, read from the capability matrix above. Name them explicitly so the user knows what they've opted into. For example:
  ...
  - `gh-issue`: "`gh-issue` supports: /add-task, /list-tasks, /promote-tasks, /do-tasks (single and batch /do-tasks --all — one remote session per dependency-ready issue, bounded by the WIP limit), /archive-tasks (hygiene only — GitHub has no issue cap; it just labels long-closed issues). You can still manage these in GitHub directly."
