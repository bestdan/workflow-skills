...
`/do-tasks` is the single execute verb across handlers: executes a single task by default, `--all` / `-n N` for batch dispatch on `repo-pr`, `linear`, and `gh-issue` (each dispatches one remote session per dependency-ready issue/task, bounded by the WIP limit). On `jira` it claims and executes one issue in the current session.
