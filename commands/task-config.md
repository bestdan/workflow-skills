...
`reoptimize` (`/reoptimize-tasks`) is the exception that runs _against_ the tracker graph rather than the local files. **`linear`** applies relation edits, priority, and duplicates natively. **`gh-issue`** is `report` — GitHub Issues **does** have a native `blocked_by` edge (`/push-plan` writes one per plan dependency and `/do-tasks` reads it), but this flow has not been taught to write one yet, so dependency findings surface as a report plus a suggested `Blocked by:`/`Related:` body footer line rather than a real edge; priority/label fixes still apply via `gh issue edit`. `jira` re-optimize is a planned follow-up, and `repo-pr` is `no` because its tasks are local plan files — re-optimize those directly (or `/push-plan` to a tracker first).
