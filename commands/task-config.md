...
- **The handler's supported and unsupported verbs**, read from the capability matrix above. Name them explicitly so the user knows what they've opted into. For example:
  ...
  - `gh-issue`: "`gh-issue` supports: /add-task, /list-tasks, /promote-tasks, /do-tasks (single; batch /do-tasks --all is opt-in via `gh-issue.remote_batch: true`, and leave it off unless your cloud environment installs this plugin itself and gives sessions a working gh credential — a committed .claude/settings.json gave neither, probed 2026-09-05), /archive-tasks (hygiene only — GitHub has no issue cap; it just labels long-closed issues). You can still manage these in GitHub directly."
