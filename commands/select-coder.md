---
description: Recommend which coder agent and model should execute a coding task — probes locally available backends (opus, codex, agy, devin, or a configured CAO custom coder such as cao-codex / cao-agy) once into a cached config, scores the task against a capability matrix (correctness, speed, cost, context, creativity, autonomy, verification, secret exposure, containment), and returns ranked coder specs; usable standalone or as orchestrate-coders --coder arguments
