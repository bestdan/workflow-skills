...
| **`/do-tasks`**      | `/do-tasks [slug \| --all \| -n N] [--remote\|--local]`                                                                       | Primary execute verb. Resolves the handler and turns dependency-ready tasks into PRs. For `repo-pr` it subsumes `/process-tasks` (default = highest-ranked task; `--all` / `-n N` batch bounded by `wip_limit`; `--local` runs in-session). Tracker handlers defer to `/claim-task` for now. |
...
| **`task` skill**     | mentions of "task", "todo", "follow-up", "we should come back to this", or any of the `/add-task` / `/process-tasks` commands | Auto-trigger skill: provides the task file format, capture workflow, handler abstraction, and processing logic. Loaded in-context whenever Claude sees deferred-work language.                                                                                                               |
