...
| **`/archive-tasks`**       | `/archive-tasks [--older-than <N>d] [--issues <refs>] [dry-run]`                                                         | Retire terminal-state work past an age threshold, handler-dispatched. `repo-pr` moves stale `done` files to `dev_docs/tasks/_archive/`; `linear` keeps a workspace under its 250-active-issue cap via native auto-archive + a GraphQL `issueArchive` backstop; `gh-issue` labels long-closed issues (hygiene only); `jira` transitions terminal issues to a configured `archive_status` (else no-op). A sweep refuses to mutate without a threshold (no surprise bulk archive); `--issues PRE-12,PRE-13` (`linear` only) archives named items whatever their age — still terminal-state-only. |
| **`/complete-task`**       | `/complete-task <identifier> [--dry-run]`                                                                                | Explicitly transition one identified work item to its tracker's completed state. Supports **`linear`** and **`gh-issue`** (v1); `repo-pr`/`jira` unsupported — see the handler file. A handler-dispatched primitive that trusts the caller and does no PR/merge verification of its own.                                                                                                                                                                                                                                                                                                      |
| **`/sweep-for-complete`**  | `/sweep-for-complete [--apply] [--all] [--project <id\|name>]`                                                           | Safe, schedulable sweep that finds started-state issues whose linked PR merged and completes exactly those via the `/complete-task` primitive. Default scope is your configured `linear.projects` plus project-less issues only — not every unrelated project on the team; `--all` widens to the whole team, `--project` narrows to one. **linear-only (v1)**; composes with `/loop` or `/schedule`.                                                                                                                                                                                          |
| **`/reconcile-tasks`**     | `/reconcile-tasks [--apply] [--all] [--project <id\|name>]`                                                              | Bounded, schedulable reconciler that fixes issues sitting in the wrong state against a fixed rule table. Same scope defaults as `/sweep-for-complete` (`--all` widens, `--project` narrows). **linear-only (v1)**; composes with `/loop` or `/schedule`.                                                                                                                                                                                                                                                                                                                                      |
...
| Skill            | Trigger                                                                                                                | What it does                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ---------------- | ---------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **auto-pilot**   | `/auto-pilot <linear-project \| plan-dir>` — advance a workstream independently, no person in the loop _(in progress)_ | Advance a whole task graph unattended: an isolated worktree, a thin orchestrator walking the graph, and `/deliver-task` per task (claim → implement → PR → co-review → hand-off) with durable, crash-resumable state. v1 is being built — see [the design decisions](dev_docs/auto-pilot.md); this release ships the skill home and the [run-state reference](skills/auto-pilot/references/run-state.md).                                                                                                                                                                                     |
