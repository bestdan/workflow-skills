...
6 skills, 7 commands, and 1 subagent, organized into four workflows:
...
| **`/do-tasks`**      | `/do-tasks [slug \| --all \| -n N] [--remote\|--local]`                                                                                     | Primary execute verb. Resolves the handler and turns dependency-ready tasks into PRs. For `repo-pr` it subsumes `/process-tasks` (default = highest-ranked task; `--all` / `-n N` batch bounded by `wip_limit`; `--local` runs in-session). For `linear` it subsumes `/claim-task`: claims and executes one issue in the current session (single/foreground, pre-claim `wip_limit` gate). jira/gh-issue have no execute path yet. |
| **`/process-tasks`** | `/process-tasks [slug \| --all \| --local]`                                                                                                 | Claim and execute dependency-ready `repo-pr` tasks. Default dispatches the highest-priority eligible task to a remote Claude session; `--all` dispatches every eligible task and skips ones still blocked by another task; `--local` processes in the current session.                                                                                                                                                            |
...
| **`/claim-task`**    | `/claim-task [issue id]`                                                                                                                    | Find a task small enough to finish in this session, claim it, branch, code, and open a PR — dispatches to the configured handler (Linear today; file-based defers to `/process-tasks --local`).                                                                                                                                                                                                                                   |
| **`task` skill**     | mentions of "task", "todo", "follow-up", "we should come back to this", or any of the `/add-task` / `/do-tasks` / `/process-tasks` commands | Auto-trigger skill: provides the task file format, capture workflow, handler abstraction, and processing logic. Loaded in-context whenever Claude sees deferred-work language.                                                                                                                                                                                                                                                    |
