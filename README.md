...
**Approve the reviewer commands once.** co-review sends the rubric, your extra requests, and the diff to local reviewers on **stdin** with a fixed prompt (passed as an argument, or — for stdin-only agents like devin — folded into the stream), so the command never changes — you approve each reviewer once with an exact-match, read-only rule (no broad wildcard). Merge them into the `permissions.allow` array in `~/.claude/settings.json` (or the repo's `.claude/settings.json`) — don't paste over an existing file. The co-review skill's **Permissions** section has the exact JSON to copy.
