...
- The `agy`/`devin`/`crush`/`grok` probe rules (`Bash(agy models)`, `Bash(devin auth status)`, `Bash(crush --version)`, and grok's `Bash(grok --version)` plus its `grep -c` telemetry-pin check) are read-only status queries with no varying arguments — exact-match, safe to approve once. grok's pin check only reads `~/.grok/config.toml` and counts three **anchored** assignments (the anchoring is load-bearing — see [`reviewers/grok.md`](../reviewers/grok.md)); it is the gate that keeps a reviewed diff out of xAI's trace channel, so approve it or grok skips every run. `copilot` has no probe rule (no `auth status` command; failures are caught from output).
