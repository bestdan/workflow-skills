...
- **exit 1 (`absent`)** → the branch is checked out nowhere, so this tree is on something else (the `head=` field names what). Stop. In the main checkout, offer to check it out here. **A worktree-isolated session cannot reach it at all** — no tool lands a session on an existing branch — so there, offer `--post`, which needs no branch: [`references/reaching-the-prs-branch.md`](references/reaching-the-prs-branch.md). Don't review the tree you happen to be in.
- **exit 3 (`unknown`)** → `git worktree list` failed. Warn, proceed, and note it in the run summary — never fatal.
