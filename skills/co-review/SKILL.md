...
- **Working directory `unknown`** (step 3, default disposition) — a **logged hard error that ends the run**, never a prompt. The check is cheap, local, and runs before any reviewer is dispatched, so stopping here wastes nothing. **`unknown` ends the run here too**, unlike the other pre-flights: step 12 pushes the current branch, so a tree that was never established cannot be shown to be the push target (log `unknown: could not establish the working tree`). Under `--local` and `--post` nothing is pushed, so it warns and continues there — and under `--local` there is no ref and so no verdict to fail on, so record the `cwd=` line in the run summary instead.
