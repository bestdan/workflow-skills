...
8. **Spawn the reconciler sub-agent** (`general-purpose`). Give it:
   ...
   - A finding that recommends pinning or SHA-pinning a GitHub Action to a version/tag is **never high confidence** unless the finding (or your own check) confirms that version is current — a reviewer endorsing a stale major version is exactly the failure mode this check exists to catch. Downgrade an unverified pin recommendation to medium and say why in the rationale.
