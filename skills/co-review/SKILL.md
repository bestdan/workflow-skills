...
- **`--local`:** no `--ref`. The check degenerates to the inventory, and that is what you want here: `--local` diffs whatever tree the session happens to be standing in, so if that is not the tree the user is editing, the review reviews the wrong content and says nothing about it. Record the `cwd=` line in the run summary so the reviewed tree is named rather than assumed.
- **`--post`:** skip it. Nothing local is read as context for a judgment and nothing is written, so where you stand doesn't matter.
...
12. **Commit and push the changes.** Once the fixes are applied and verified, commit them and push to the current branch's upstream:
    ...
    - **Quote the `git status --porcelain` / `git diff --stat` evidence from step 10 in the summary**, alongside the commit SHA and the tree it was made in. A list of applied fixes with no diff behind it is a claim, not a report.
