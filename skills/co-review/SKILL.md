...
- Architectural/design judgment calls (signal handling, test design patterns, harness structure, and similar calls with a defensible "right" answer) get escalated to an independent, non-authoring subagent (fresh context; only the item, its hunk and the files it cites) for a recommendation before they reach the user as a question; genuine user-preference calls (priority, scope, whether to do the work) go to the user directly. See step 9.
