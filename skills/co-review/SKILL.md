...
- **exit 0 (`ok`)** → proceed.
- **exit 1 (`foreign`)** → **stop. Enter the worktree; do not reach into it.** On a harness with a worktree tool (Claude Code: `EnterWorktree` with the `path` from the `worktree=` field), enter it and re-run from there. Where there is no such tool, tell the user the path and let them re-run the command from inside it. Do **not** carry on with `cd <path> && …` or `git -C <path>` — the prefix is exactly the failure, not the workaround for it.
- **exit 1 (`absent`)** → the branch is checked out nowhere, so this tree is on something else (the `head=` field names what). Stop. In the main checkout, offer to check it out here; from a worktree-isolated session, offer to make a worktree for it, which needs the user to approve the path. **Unattended, take no route even where one exists** — in a worktree the approval cannot be given, and moving `HEAD` in someone's main checkout is not a default an unwatched run may pick — so `absent` stays the hard error below and `--post` is what a human re-runs with, never an automatic switch: [`references/reaching-the-prs-branch.md`](references/reaching-the-prs-branch.md). Don't review the tree you happen to be in.
...
Under `--non-interactive`, `foreign`, `absent` and — in the default disposition — `unknown` are **logged hard errors that end the run**, not prompts — see **Non-interactive mode**.
...
- **Working directory `foreign` / `absent` / `unknown`** (step 3, default disposition) — a **logged hard error that ends the run**, never a prompt, and never a silent `git -C` fallback. The unattended caller has no one to enter the worktree for it, and the two failures it would otherwise buy are both invisible: a reconciler judging the diff against the wrong tree, and fixes committed and pushed to a tree nothing else in the system is looking at. Log `foreign: <headRefName> is checked out at <worktree>; re-run /co-review from there` (or `absent: <headRefName> is checked out in no worktree`). The check is cheap, local, and runs before any reviewer is dispatched, so stopping here wastes nothing. **`unknown` ends the run here too**, unlike the other pre-flights: step 12 pushes the current branch, so a tree that was never established cannot be shown to be the push target (log `unknown: could not establish the working tree`). Under `--local` and `--post` nothing is pushed, so it warns and continues there — and under `--local` there is no ref and so no verdict to fail on, so record the `cwd=` line in the run summary instead.
