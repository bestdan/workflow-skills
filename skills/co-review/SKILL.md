...
**Detection (PATH probe + config override):** the known default list is `codex`, `agy`, `devin`, `copilot`, `crush`, and `grok` (`gemini` is retired — skip it if present). Probe each with `command -v`. The config may also name agents that aren't on the default list, supplying a `command:` for how to invoke them.
