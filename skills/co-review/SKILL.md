...
local_reviewers:
  ...
  - name: devin # object form for a built-in agent → optional model override
    model: swe-1.6 # only needed to override devin's default (swe-1.6-slow); requires a paid tier
...
> - **Pin `--model`; default `swe-1.6-slow`.** The built-in invocation defaults to **`swe-1.6-slow`** because it is the model reachable on the **Devin Free tier** — on Free, the frontier models (`swe-1.6`, `swe-1.6-fast`, `gpt-5.2`, `gemini-3-flash`, every `claude-*`, `glm-5.2`, `kimi-k2.7`) all return `/upgrade to access this model`, and the unpinned default panics. `swe-1.6-slow` is Cognition's own SWE model — a voice distinct from Claude (the main agent), OpenAI (`codex`), and Gemini (`agy`), which is the point of a second reviewer — and in testing it flagged an injected bug correctly in ~3.5s. On a paid tier, pin a faster/heavier model (`swe-1.6`, `swe-1.6-fast`, `gpt-5.2`, …) via the config object form `- {name: devin, model: swe-1.6}`; changing the model changes the command string and re-prompts the exact-match approval.
...
- `devin`, GitHub mode, **with** requests → `cat "<this skill dir>/review_prompt.md" "<REQUESTS>" > "<INPUT>"; gh pr diff <n> >> "<INPUT>"; [ -s "<INPUT>" ] && devin -p --prompt-file "<INPUT>" --model "swe-1.6-slow" --sandbox --permission-mode auto` — devin reads the assembled `rubric + requests + diff` from the file via **`--prompt-file`** (there is **no stdin pipe** and **no `-p "<POINTER>"` argument** — piping stdin panics and an inline prompt suppresses the file; the rubric itself carries the read-only instruction). The leading `[ -s "<INPUT>" ] &&` is the same empty-input guard used for `agy`. `--sandbox` + `--permission-mode auto` enforce read-only at the OS and tool layers; `--model "swe-1.6-slow"` is the Free-tier-reachable pin (swap to `swe-1.6`/`gpt-5.2`/… on a paid tier via the config object form, which re-prompts the exact-match approval). devin needs network + a Devin login, so this line must run **unsandboxed** in the Bash tool. **Never** add `-c`/`--continue`/`-r`/`--resume` — every review is a fresh session.
- `devin`, GitHub mode, **no** requests → drop the `"<REQUESTS>"` argument: `cat "<this skill dir>/review_prompt.md" > "<INPUT>"; gh pr diff <n> >> "<INPUT>"; [ -s "<INPUT>" ] && devin -p --prompt-file "<INPUT>" --model "swe-1.6-slow" --sandbox --permission-mode auto`
...
```json
{
  "permissions": {
    "allow": [
      ...
      "Bash(devin -p --prompt-file \"<INPUT>\" --model \"swe-1.6-slow\" --sandbox --permission-mode auto)"
    ]
  }
}
```
...
5. **Dispatch local-agent reviews** (if any) **in parallel.** Assemble the shared `<INPUT>` file (rubric + any reviewer-specific requests + diff) and hand it to each agent exactly as described under **Local reviewers → Built-in invocations** — piped on stdin with the fixed pointer prompt for `codex`/`agy`, or via `--prompt-file "<INPUT>"` for `devin`; capture stdout. For any custom `command:` or non-built-in agent, show the command and get explicit user confirmation before the first run (untrusted config; see the note in Local reviewers). If an agent errors, times out, or isn't actually runnable, note it and continue — a missing reviewer is not fatal. Output is free-form prose; do not impose a JSON contract on external tools.
