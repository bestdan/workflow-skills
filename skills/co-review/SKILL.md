...
- Exit `1` → `AGENT_GUIDANCE_DIR` is set but does not name a plugin root. That is a misconfiguration, not an absence, so do not fall through as if nothing happened: interactively, stop and show the user the stderr before dispatching anything; under `--non-interactive`, dispatch without the segment and put the stderr verbatim on the Reviewers line, so the caller sees it.
