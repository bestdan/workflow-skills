...
- **exit 1 (`absent`)** → the branch is checked out nowhere, so this tree is on something else (the `head=` field names what). Stop. In the main checkout, offer to check it out here; from a worktree-isolated session, offer to make a worktree for it, which needs the user to approve the path. **Unattended, neither is available** — no tool lands a session on an existing branch and the approval cannot be given — so there `--post` is the only disposition left: [`references/reaching-the-prs-branch.md`](references/reaching-the-prs-branch.md). Don't review the tree you happen to be in.
...
- **Default (your PR):** the PR's head branch (`headRefName`) — step 12 pushes fixes to it, so a stale local copy means a rejected push or clobbered upstream commits. Deliberately not "the current branch", which is only usually the same ref: [`references/reaching-the-prs-branch.md`](references/reaching-the-prs-branch.md).
