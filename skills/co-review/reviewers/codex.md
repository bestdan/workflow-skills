...
>
> **Recommended: pin the model** (see below). Until you do, read the default off your resolved config rather than assuming it: `codex config get model`, which is the same lookup [`scripts/probe-coders.sh`](../../../scripts/probe-coders.sh) uses, falling back to the `model =` line in `~/.codex/config.toml` when it prints nothing.
...
> **Verify the flag — and the model — before you add the rule.** Unlike the rest of this file, the `--model` spelling above was **not** confirmed against a running binary — check `codex exec --help` on your installed version first. If the flag is wrong the command fails; per the dispatch contract co-review then **notes the reviewer as skipped in the run summary** and continues (never fatal). So the failure is reported, not silent — but it is easy to miss, because losing a reviewer doesn't stop the run. Check the summary rather than assuming codex ran. The same goes for the model you pin: codex is the one reviewer co-review dispatches with **no pre-flight probe** (`agy` gates on `agy models`, `devin` on `devin auth status`), so a model your account cannot serve gives no early signal at all — just a reviewer that drops out of every run. Pinning also changes the command string, so it **re-prompts the exact-match approval** and the allow-rule below must be updated in lockstep.
