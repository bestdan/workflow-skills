...
## Why `gpt-5.6-terra`, and what it cost to get here
...
**`gpt-5.5` was the integrity-conservative pick, and it stopped being servable.** `matrix.md` lists it as "the **integrity-conservative** pick — substitute for Sol when a self-report must be trusted," and its **Terminal-Bench 2.1 83.1%** carries neither caveat marker — no `*` (vendor-reported), no `‡` (absent from the primary board). That reasoning still holds. What changed is availability: a dispatch now returns
...
**Never pin `gpt-5.6-sol`** for a review, per the note above. Never drop `--model` either: unpinned resolves to the config default, which on a common setup _is_ Sol. If `gpt-5.5` becomes servable on your account again, moving back is a strict improvement — check with `codex doctor` (below) rather than assuming either way.
