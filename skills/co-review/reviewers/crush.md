...
- **No pre-flight probe.** Unlike `agy` (30s doomed OAuth) and `devin`, crush fails **fast** — a misconfigured model errors in ~0.8s with an `ERROR` block and a non-zero exit. It gets the `copilot` treatment: no probe, failure caught from output at dispatch.
...
- **GitHub mode, with requests** → `mkdir -p "<NEUTRAL>"; cat "<this skill dir>/reviewers/assets/crush-readonly.json" > "<NEUTRAL>/crush.json"; cat "<this skill dir>/review_prompt.md" "<REQUESTS>" > "<INPUT>"; gh pr diff <n> >> "<INPUT>"; cat "<INPUT>" | crush run --cwd "<NEUTRAL>" -q -m "hyper/kimi-k2.7-code" "<POINTER>" 2>&1`
- **GitHub mode, no requests** → drop the `"<REQUESTS>"` argument: `mkdir -p "<NEUTRAL>"; cat "<this skill dir>/reviewers/assets/crush-readonly.json" > "<NEUTRAL>/crush.json"; cat "<this skill dir>/review_prompt.md" > "<INPUT>"; gh pr diff <n> >> "<INPUT>"; cat "<INPUT>" | crush run --cwd "<NEUTRAL>" -q -m "hyper/kimi-k2.7-code" "<POINTER>" 2>&1`
...
Merge into the `permissions.allow` array (see [`../references/permissions.md`](../references/permissions.md)). The first prepares the neutral cwd (it is the same rule `devin` uses — one entry serves both), the second is the reviewer command. Replace `<NEUTRAL>` with a literal fixed absolute path, the same one the invocation uses. The config copy is covered by the shared `Bash(cat:*)` rule.
