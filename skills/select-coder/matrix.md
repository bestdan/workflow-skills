...
- **Terminal-Bench 2.1 (tbench.ai)** is the trustworthy column. It is
  harness-explicit — every row names the agent *and* the model, which is the
...
ultimately serves this request" now varies *per model* inside one backend, and
...
*every* Devin model — not just the China-lab ones — you cannot answer "whose
...
Fable 5's 83.8% is the highest number on the *official* board and is the one to
...
- `codex:gpt-5.5` is promoted to 2nd on `architecture` and 3rd on
  `long-horizon` over `gpt-5.6-sol`, because 5.5 is the highest *benchmarked*
