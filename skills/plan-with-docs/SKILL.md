...
1. **Resolve `<name>`.**
   ...
   - **Ask about the merge model before writing anything, if context suggests it might not be the standard flow** — e.g. the user has floated a plan-branch, a "throwaway" branch, or a workflow where this plan's commits will land some other way. Ask: "Should the plan stay on `main` as permanent scaffolding, or live only on a plan-branch that never merges?" The answer decides whether the plan's final task is the usual graduate-then-delete cleanup or something else (see **Rules**) — don't bake in a deletion task that assumes a merge that will never happen. Skip the question when nothing in the conversation suggests a non-standard flow; the standard merge-to-`main` assumption is the default.
