...
1. **Resolve `<name>`.**
   ...
   - **Ask about the merge model before writing anything, if context suggests it might not be the standard flow** — e.g. the user has floated a plan-branch, a "throwaway" branch, or a workflow where this plan's commits will land some other way. Ask: "Will this plan land on `main` through the normal merge flow, or live only on a plan-branch that never merges?" The plan folder is temporary either way — this only decides _how_ it gets cleaned up. The answer decides whether the plan's final task is the usual graduate-then-delete cleanup or something else (see **Rules**) — don't bake in a deletion task that assumes a merge that will never happen. Skip the question when nothing in the conversation suggests a non-standard flow; the standard merge-to-`main` assumption is the default.
...
8. **Review with the user.** Surface every open question and judgment call you noted while drafting — assumptions you made, alternatives you considered and rejected, gaps in your understanding of the codebase. Before this review, run one explicit consistency pass over every numeric or resource constraint the plan states — a size budget, a rate limit, a timeout, a quota — across `<name>_plan.md` and every task file; a contradiction caught here is a question surfaced now, not a wasted round trip an implementer hits mid-build. Fold any contradiction you find into the same open-questions list. Ask them one by one (or grouped if tightly related). Update the affected files in place as the user answers; don't make the user restart the planning round-trip. Stop when the user says the plan is good.
...
- **Make the plan's last task the graduate-then-delete cleanup** (see the plan-lifecycle note near the top): a final task, `is_blocked_by` the plan's terminal task(s), that migrates the durable architecture/decisions into a permanent `dev_docs/<name>.md` and deletes the temporary `<name>_plan/` folder and any design/notes files the plan spawned. A plan isn't done until its scaffolding is gone. **This applies when the plan lands on `main` through its normal merge flow.** If step 1's merge-model question established that the plan lives only on a branch that never merges to `main`, don't write a task that assumes deletion via merge — either omit the cleanup task or write one suited to that branch's own lifecycle.
