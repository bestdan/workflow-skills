...
**Subsystem count is a split signal independent of raw line/file count.** A task can stay under the size-5 ceiling by lines/files while still crossing several unrelated subsystems — e.g. a sync/build script, a test suite, formatter/tooling config, and domain logic/routine definitions, in any combination. Treat **3+ unrelated subsystems** as a split trigger on its own: default to splitting along subsystem boundaries unless each component is individually small (size ≤2 on its own). `skills/assess-task/SKILL.md`'s `scope` dimension and `skills/break-down-task/SKILL.md`'s shear-point detection both apply this signal — see those skills for how each surfaces it.
