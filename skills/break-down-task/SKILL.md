...
4. **Remove or convert the original task file.** If you kept it as a thin epic parent (step 2), convert its frontmatter to the epic shape (`type: epic`, `status: active`) instead of deleting — the components' `parent` references need it to exist, and an epic file must carry a valid epic `status`. Otherwise remove it (`git rm` or delete in the working tree); the components now carry all its scope.
