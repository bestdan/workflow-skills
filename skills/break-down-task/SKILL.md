...
4. **Remove or convert the original task file.** If you kept it as a thin epic parent (step 2), convert its frontmatter to `type: epic` instead of deleting — the components' `parent` references need it to exist. Otherwise remove it (`git rm` or delete in the working tree); the components now carry all its scope.
...
6. **Report.** Print the original slug and its fate (removed, or converted to epic), the new component slugs with their `is_blocked_by` edges (a tiny dependency tree), and remind the user to run `/promote-tasks` to score the new cards and `/list-tasks` to see the board.
