...
urgent_check="$(printf '%s' "$out7" | jq -r '.cards.new[] | select(.slug=="urgent") | .promote_gate.checks | has("priority_not_urgent")')"
...
assert_eq "promote gate: priority_not_urgent check no longer exists" "false" "$urgent_check"
