...
# The supervisor broker: verify, status, teardown, run-head, exit classification, and the pause gate.
#
...
#
# Run directly: bash scripts/test-spawn-orchestrator-supervisor.sh
set -uo pipefail
SO_PART=supervisor
...
# _gate_iso and NOW_EPOCH come from the prelude — the exit-contract part reads
# both, so they could not stay here.
