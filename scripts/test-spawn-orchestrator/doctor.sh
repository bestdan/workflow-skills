...
# Run invariant audit (task 14, generalizing findings #22/#23).
#
...
# files. Runnable on its own: bash scripts/test-spawn-orchestrator/doctor.sh
...
GHFAILEOF
  ...
  # <30, not tighter: the failure mode this pins is the watchdog holding the
  # pipe for the FULL 60s bound, so half that still separates the two cleanly —
  # and the suites now run concurrently, where a descheduled sub-second scan
  # can stretch by seconds without anything being wrong (see 1ddb13e).
  [ "$scan_elapsed" -lt 30 ] \
