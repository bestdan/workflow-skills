...
*A plain-language tour of how `workflow-skills` turns "we should come back to
...
and opens a PR for review — all without losing the context of *why* the task
...
| ----------------- | ------------------------------------------------------------------------------- |
| **`/add-task`**   | Capture follow-up work *with full context* and file it to the configured place. |
...
| **`/task-config`**| Choose *where* tasks live (repo files, GitHub Issues, Jira, or Linear).         |
...
- Drafts a structured card — title, priority, a **size estimate** (Fibonacci
  1/2/3/5), the relevant files, a *Context* section ("written for someone who
  has never seen this code"), concrete *Task* steps, and *Acceptance Criteria*,
- Shows it to you for a quick edit, then files it.

The card is born `status: new`. Crucially, capture preserves the *why* — the
...
- Does it have acceptance criteria and *no* unresolved open questions?
...
| ---------- | -------------------------------------------- | ------------------------------------------- |
| `repo-pr`  | a markdown file committed via PR *(default)* | self-contained, no external tooling         |
...
**Capability is jagged** — only `repo-pr` runs the *full* loop today:
...
- `done` → **Done**, set *only* by Linear's GitHub integration on PR merge.
...
Two skills feed *into* the same pipeline by emitting ordinary `new` cards:
...
- **`plan-with-docs`** — turns a fresh, multi-PR idea into a directory of
  ...
  backlog.
- **`break-down-task`** — does the same slicing for an *existing* card that grew
  too big.
...
`ready` card**. The lock is *not* a branch name (which fails in branch-pinned
...
- **`analysis-pipeline` / `review-facts`** — a separate family for *auditable*
