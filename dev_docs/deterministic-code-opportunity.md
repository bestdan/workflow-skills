...
The second prose-to-code round — sourced from a 2026-09-09 survey (last
readable in git history at
[`2026-09-09-prose-to-code-index.md`](https://github.com/bestdan/workflow-skills/blob/c1af35eb26af8d614cf2fa93ebd41b6e919c4af1/dev_docs/2026-09-09-prose-to-code-index.md))
via [milestone 2](https://github.com/bestdan/workflow-skills/milestone/2) —
shipped 14 of its 15 tasks (the 15th, this section itself, is issue #538). The
survey's own point-in-time index file is deleted (it was a snapshot, not
durable guidance — see "How to run the next survey" below for the durable
method); the table below links each shipped script directly to its PR. The
survey's unshipped rows are tracked as GitHub issues under the
"Prose to code, round 3 candidates" milestone rather than restated here.

### Shipped scripts (final interfaces)

| Script                                                              | PR(s)                                                                                                                    | Interface                                                                                                                                                                                                                                    | Replaces (re-derived prose)                                                                                                                   |
| ------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| `commands/handlers/assets/jira-resolve-transition.py`               | [#545](https://github.com/bestdan/workflow-skills/pull/545)                                                              | `jira-resolve-transition.py [--category C [--exclude RE] [--prefer RE]] \| --exact NAME` — transitions JSON on stdin → `<id>\t<to.name>` on success, or `AMBIGUOUS`/`NONE` with candidate names                                              | Jira transition-id resolution restated at 5 call sites (`jira-claim.md`, `jira-complete.md`, `jira-promote.md`, `jira.md`, `jira-archive.md`) |
| `scripts/diff-anchor-check.py`                                      | [#546](https://github.com/bestdan/workflow-skills/pull/546)                                                              | `diff-anchor-check.py --diff <file>` — candidate-comment JSON on stdin → `{anchored: [...], unanchored: [...]}`                                                                                                                              | co-review's batched-review-POST anchor check                                                                                                  |
| `scripts/tutorial-root-guard.sh`                                    | [#544](https://github.com/bestdan/workflow-skills/pull/544)                                                              | `tutorial-root-guard.sh <path>` — exit 0 if safe, exit 1 with the failing predicate on stderr                                                                                                                                                | research-spike-tutorial's hand-walked `$WORK` root guard ahead of an `rm -rf`                                                                 |
| `commands/handlers/assets/linear-graph-analyze.py`                  | [#541](https://github.com/bestdan/workflow-skills/pull/541)                                                              | `linear-graph-analyze.py [--file <linear-relations.py JSON>]` (else stdin) → cycles, topological order, priority inversions                                                                                                                  | `linear-reoptimize.md`'s hand-walked cycle detection, topo sort + tie-break, priority-inversion sweep                                         |
| `commands/handlers/assets/gh-issue-graph.py --sort`                 | [#542](https://github.com/bestdan/workflow-skills/pull/542)                                                              | adds `--sort` (and `--edges <file>`) to the existing `gh-issue-graph.py --repo <o/n> [...]` → topological order over open, in-scope nodes                                                                                                    | `gh-issue-reoptimize.md` Dimension 3's hand-walked topo sort                                                                                  |
| `commands/handlers/assets/_body_refs.py`                            | [#547](https://github.com/bestdan/workflow-skills/pull/547)                                                              | `parse(body, self_id, id_pattern) -> [{target, phrase, direction, strength}]`; called from `gh-issue-graph.py` and `linear-relations.py`, which each gain a `body_references`/`proposed` field                                               | dependency-phrase body parsing with a direction table, duplicated in `gh-issue-reoptimize.md` and `linear-reoptimize.md`                      |
| `commands/handlers/assets/linear-pr-resolve.py`                     | [#550](https://github.com/bestdan/workflow-skills/pull/550), [#554](https://github.com/bestdan/workflow-skills/pull/554) | `linear-pr-resolve.py [--config <.task-config.yml>] [--repo <o/n>]` → per-issue `{prs, resolved_via, unresolved}`; `classify()` → `merged`/`open`/`unresolved`/`closed_unmerged`/`no_pr`                                                     | `linear-sweep-complete.md`'s three-source PR-discovery choreography and merge-state precedence                                                |
| `scripts/spawn-orchestrator.sh reserve-gate`                        | [#551](https://github.com/bestdan/workflow-skills/pull/551)                                                              | `reserve-gate --run-md <p> --percent <p> --reset-epoch <e> [--floor n] [--samples n]` → `RESERVE: reserve=n headroom=n verdict=proceed\|pause samples=n`; `--read-failed` → `RESERVE: reserve=n headroom=unknown verdict=fallback samples=n` | `run-budget.md`/`run-state.md`'s `usage_delta` reserve bookkeeping, hand-walked at four `/deliver-task` boundaries                            |
| `commands/handlers/assets/gh-issue-claim.py acquire-ref`            | [#549](https://github.com/bestdan/workflow-skills/pull/549)                                                              | `acquire-ref --repo <o/n> --branch <name> --base-sha <sha>` (exit 0 won / 3 lost / 4 other), sharing `acquire_ref()` with the existing `acquire` subcommand                                                                                  | `claim-lock.md` and `jira-claim.md`'s hand-walked create-only-ref POST/422 dispatch                                                           |
| `scripts/preflight.sh --scout-run-md`                               | [#557](https://github.com/bestdan/workflow-skills/pull/557)                                                              | `preflight.sh --scout-run-md <RUN.md path>` → `SCOUT VERDICT: go` or one or more `BLOCKS LAUNCH:` lines                                                                                                                                      | auto-pilot's task-backend-in-environment-fingerprint check, duplicated in `launch-preflight.md` and `resume.md`                               |
| `commands/handlers/assets/linear-false-closures.py --from-mcp-json` | [#552](https://github.com/bestdan/workflow-skills/pull/552)                                                              | `--from-mcp-json <search.json> <list.json> --complete-since <iso> --prs-file <out\|->` → builds the `--prs-file` JSON by joining on PR number                                                                                                | nightly-linear-tidy.md's merged-PR file builder (page, join, field-map, null-coerce)                                                          |
| `scripts/analysis-pipeline/check-reproducibility.sh`                | [#553](https://github.com/bestdan/workflow-skills/pull/553)                                                              | `check-reproducibility.sh <dir>` → `REPRO: verdict=pass\|fail\|n/a` (plus a diff on fail); exit 0 pass/n-a, 1 fail, 2 usage error                                                                                                            | `agents/fact-reviewer.md`'s clean-check / re-run / diff / guaranteed-restore sequence                                                         |
| `commands/handlers/assets/kanban-classify.py`                       | [#548](https://github.com/bestdan/workflow-skills/pull/548)                                                              | `kanban-classify.py --tracker {linear,gh-issue,jira}` — issue JSON on stdin → classified, ordered sections                                                                                                                                   | the kanban section-classification table restated in `linear-list.md`, `gh-issue.md`, `jira.md`                                                |
| `commands/handlers/assets/_linear_rank.py` (+ `linear-rank.py`)     | [#543](https://github.com/bestdan/workflow-skills/pull/543)                                                              | `_linear_rank.py`: `gate(issue, max_estimate)` / `rank_key(issue)`, shared by `linear-ready.py` (GraphQL fast path) and the new `linear-rank.py [--max-estimate N] [--viewer-id ID]` (MCP-floor, `list_issues` JSON on stdin)                | the ready-candidate gate + rank restated across `linear-claim.md`, `linear-list.md`, `linear-promote.md`, `linear-reoptimize.md`              |
...
3. **`gh-issue-graph.py --sort`'s `compare_nodes()` is still a per-pair
   comparator, but on a fixed, transitive precedence (#542).** An earlier
   per-pair "fall to age when either side lacks an estimate" form was not
   transitive (three same-priority nodes could rank A<B, B<C, and C<A), so
   the shipped comparator instead decides every pair by the same fixed
   precedence — `(priority, has_estimate, estimate, age, number)` — which is
   why has-estimate nodes group ahead of lacks-estimate ones within a
   priority tier rather than interleaving by age. The fix is the fixed
   precedence, not dropping the comparator shape.
...
The durable method behind Round 2, extracted from the now-deleted index so a
future survey doesn't have to re-derive it. The unshipped rows themselves are
tracked as GitHub issues under the "Prose to code, round 3 candidates" milestone
— this doc keeps only the method, not the row list.

**Extraction rubric.** Read every assigned file in full against one rubric:
hand-walked algorithms; fenced blocks with logic; decision tables; API
choreography; rules that could be hooks or lints; prose re-deriving a shipped
script; the same procedure restated across handlers. Re-check every absence
claim that ranks a finding — "no script computes this" — with `rg` against
`scripts/` and `commands/handlers/assets/` before ranking it, not only on the
extractor's say-so.

**Ranking criteria.** Rank weighs: an explicit prior defect in the prose,
confidence that the logic is deterministic, number of call sites, and size.

**The one standing constraint.** MCP calls (`mcp__linear__*`,
`mcp__atlassian__*`) run only from the agent, so a script can own the
**decision** over fetched JSON but never the fetch or the write. Most findings
are shaped that way on purpose.

**Do not re-propose.**

- Research-spike decision promote (cut block, flip state, paste),
  `skills/research-spike/SKILL.md:243-252`. **Do not extract.**
  `scripts/research-spike.py:104-110` and `:1925-1931` reserve promotion to the
  organizer by design.
- Reviewer input assembly repeated in five reviewer files,
  `skills/co-review/reviewers/*.md`. **Do not collapse.** `SKILL.md` keeps the
  literal command in each file so the exact-match permission rule can approve
  it byte-for-byte.
