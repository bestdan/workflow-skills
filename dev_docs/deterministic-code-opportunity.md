...
| `commands/handlers/assets/linear-pr-resolve.py`                     | #550, #554 | `linear-pr-resolve.py [--config <.task-config.yml>] [--repo <o/n>]` → per-issue `{prs, resolved_via, unresolved}`; `classify()` → `merged`/`open`/`unresolved`/`closed_unmerged`/`no_pr`                                                     | `linear-sweep-complete.md`'s three-source PR-discovery choreography and merge-state precedence                                                |
| `scripts/spawn-orchestrator.sh reserve-gate`                        | #551       | `reserve-gate --run-md <p> --percent <p> --reset-epoch <e> [--floor n] [--samples n]` → `RESERVE: reserve=n headroom=n verdict=proceed\|pause samples=n`; `--read-failed` → `RESERVE: reserve=n headroom=unknown verdict=fallback samples=n` | `run-budget.md`/`run-state.md`'s `usage_delta` reserve bookkeeping, hand-walked at four `/deliver-task` boundaries                            |
