...
_without_ `--output-format`, so it green-lights an invocation shape the real run
never uses.
**Fix:** add `--verbose` to `write_launch`'s emitted command; make `smoke-test`
run the _exact_ flag set the launch script uses (verbose + stream-json), not a
reduced one.
...
- **Symlink/version drift:** `claude` → `~/.local/share/claude/versions/2.1.207`,
  ...
  next `claude update`, and a caller passing bin _dirs_ (as I did) must know to
...
tree" precisely to avoid nesting; the reality that _coders and tests_ also nest
...
made the run-state branch the lock — but it had to _derive_ that.
...
_orchestrator_ moving a base, not a _human_ merging one.
