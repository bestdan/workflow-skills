...
A snapshot. Line numbers are as of `a152b18` (v2.28.1) for this repo and the
`dotfiles/agents` tree on the same day. Nothing here is maintained; a finding
...
- **Three procedures are restated across handlers** and would each collapse
  to one tested function: Jira transition-id resolution (3 copies), the
...
| 2  | Jira transition-id resolution (category filter → exclude → prefer-name → disambiguate)        | `jira-claim.md:213-230`, `jira-promote.md:117-125`, `jira-complete.md:82-111` (+ one sentence in `jira-archive.md`)                             | `jira-resolve-transition.py`: transitions JSON on stdin + `--category`/`--prefer`/`--exclude` → id or `AMBIGUOUS`/`NONE` with names. Three call sites, one implementation.                               | S    | `jira-complete.md`: "Filtering first is what stops a board whose only terminal transition is Canceled from being quietly completed." Verified: no `jira-*.py` asset exists.             |
| 3  | Dependency-phrase body parsing with direction table                                           | `gh-issue-reoptimize.md:136-145`, `:173-182`; `linear-reoptimize.md:96-121`                                                                     | Extend `gh-issue-graph.py` and `linear-relations.py` to emit `body_references: [{target, phrase, direction, strength}]`; agent judges strength only.                                                     | M    | "a mistake here writes a real dependency backwards and nothing catches it." Verified: neither script parses bodies.                                                                     |
...
| 6  | Claim-lock create-only ref acquire (201/422/other dispatch)                                   | `commands/handlers/claim-lock.md:35-93`                                                                                                         | `claim-acquire.sh <repo> <branch> <base>` → exit 0 won / 1 lost / 2 fall back; sha on stdout.                                                                                                            | S    | "two racers both cut `<branch>` from the same tip … the loser's push reports `Everything up-to-date` and exits 0" — measured against a real remote.                                     |
...
| 35 | Key-resolution bridging ladder                                          | `commands/handlers/linear-common.md:178-219`                                                                | `_secret_resolve.py --probe` could also print the ready-to-run command line.                                                                                                                |
...
| 37 | research-spike decision promote (cut block, flip state, paste)          | `skills/research-spike/SKILL.md:243-252`                                                                    | `research-spike.py promote-decision`.                                                                                                                                                       |
...
| `gh api` method flag before the path                     | `sandbox-network-guard.sh`                                      | none                                                                                                        |
...
These overlap the index and are tracked in this repo's issues; the rows above
...
`assess-task.md`, `archive-tasks.md`, `task-config.md`, `reconcile-tasks.md`,
...
If one PR each: #2 (three call sites, S, a stated defect), #10 and #11 (S,
each guards an irreversible action), #4 (S–M, closes the largest unbacked
algorithm cluster), then #1 and #5 (the two M/L items with production
incidents behind them). #16 and #17 are the cross-handler collapses and are
worth doing together once the three tables are diffed.
