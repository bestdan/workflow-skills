...
- No backward-compat aliases for old command names or `.todo-config.yml` (clean hard rename). The only concession to existing repos is a **one-time migration prompt** (Step 5/Step 1), not dual code paths.
...
1. [[step_1]] — Rename the task skill (source of truth); add canonical **Task size** + **Legacy migration** sections.
2. [[step_2]] — Rename the six slash commands (`/add-todo` → `/add-task`, etc.).
3. [[step_3]] — Rename handler content, part 1: `repo-pr*`, `gh-issue*`, `mcp-setup-offer`.
4. [[step_4]] — Rename handler content, part 2: `jira*`, `linear*`.
5. [[step_5]] — Fold plan-with-docs into the task system + migrate the in-repo example plan + add legacy-migration preflight.
6. [[step_6]] — Consistency sweep: README, CONTRIBUTING, manifests (+ version bump), evals, validator comment; final no-stray-`todo` check.

## Decisions (resolved)

- **Command number scheme** (Step 2): mixed — singular for one-card ops (`/add-task`, `/claim-task`), plural for board/batch ops (`/promote-tasks`, `/process-tasks`, `/list-tasks`), plus `/task-config`.
- **plan-with-docs task status** (Step 5): tasks born `status: new` so dependency/blocking relationships are evaluated as they flow through `/promote-tasks`.
- **Example plan** (Step 5): fully converted to the canonical task format (not just moved), so the repo models the convention accurately.
- **Version bump** (Step 6): `0.7.0` → `1.0.0` (major — breaking rename of every command name).
- **This plan's home**: moved to `dev_docs/tasks/task_vocabulary_normalization_plan/` as part of Step 5.
- **Size field formalized** (cross-cutting; discovered during Step 2): `size` becomes a required Fibonacci story-point field (`1` / `2` / `3` / `5`); `> 5` ⇒ split into sub-tasks. Defined canonically in `skills/task/SKILL.md` **Task size** (Step 1, amended), drafted by `/add-task` and gated by `/promote-tasks` (Step 2), surfaced by `/list-tasks` (Step 2), mapped to Linear's native `estimate` in the linear handler (Step 4), and estimated per task by `plan-with-docs` (Step 5). Replaces the prior undocumented `size: small/medium/large` field.
