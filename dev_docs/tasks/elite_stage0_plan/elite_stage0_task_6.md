...
Design §7a priority 3 + §5.1. Key assumption: the inherent unattended promise — external detection and notification — works without the full registry or launcher. Falsification redirect: stop unattended work; change launch context, watcher primitive, cadence, or delivery provider and rerun. Runs in parallel with the viability canary (needs only the kill sheet, not the agent identity — it can run as the maintainer).
...
- Stand up a trivial launchd-hosted heartbeat process (touches a file per tick) and a minimal watcher launchd job at the real cadence (`StartInterval` 120s, single short-lived pass, staleness threshold 6 min).
