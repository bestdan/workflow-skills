...
- Confirm the measured-revision PR (linked from [[elite_stage0_task_8]]'s close-out) is merged — `gh pr view <that PR> --json state,mergedAt` — and that any blocked contracts carry their named redirects, as listed in the revision's blocked-contracts section.
...
- The probe-5 plan task states its input (the revision's draft launch/lease/registry state machine), its two-working-day cap (§7a row 5), and requires a pre-written kill-sheet row (falsifier, pass threshold, inconclusive condition, redirect) before any fixture.
