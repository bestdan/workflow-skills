...
2. **Single source of truth for the gates?** → A **canonical prose block in `linear-common.md`** ("Ready-candidate selection", plan task 1) owns them; the script and the MCP floor are two implementations that both cite it verbatim rather than re-deriving. (Two implementations are unavoidable — the floor must apply the gates client-side wherever the script can't run.) No automated parity check yet: the script emits its drop reasons using the block's **fixed reason strings**, which is exactly the hook a later `validate.py` parity check would diff against if the two drift in practice. _(Amended 2026-07-04 — an earlier version of this answer said "the script owns them"; the plan supersedes that: the prose block is canonical, the script is a consumer.)_
