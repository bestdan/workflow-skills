...
exactly that condition. Same shape as #22 (the breaker exists but counts the wrong
thing) and #23 (worker-worktree isolation is described but never asserted). Writing
the rule down is not the same as enforcing it; tasks 14 and 17 are what turn the
written rules into checked ones.
