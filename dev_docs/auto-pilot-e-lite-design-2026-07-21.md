...
1. **Max runaway** (no server cap): reserve gate + parallelism cap +
   ...
   control-plane ceiling on state the agent cannot write — is built. Until then,
   the only real bounds are the two out-of-process halts in `supervisor-scan`,
   one of which (blown `--until`) reads agent-writable state and fails open.
