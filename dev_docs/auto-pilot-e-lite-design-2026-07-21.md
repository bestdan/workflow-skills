...
- **Incarnation identity**: every recorded process is stored as a measured
  ...
  **`p_uniqueid`** — the monotonic, never-reused-within-a-boot kernel process id
  ...
  the runfile's claim, or the agent-owned tmux `pane_pid`. Cross-uid read
  confirmed on the mini: a non-root maintainer's `proc_pidinfo` of an agent-owned
  process is `EPERM` (fails closed, not a partial read) while root reads the full
  tuple — so the independent measurement runs **privileged (root)** alongside the
  signal, never a fallback to `ps` lstart. The matching cross-uid `killpg` drill
  is the one remaining mini confirmation.
